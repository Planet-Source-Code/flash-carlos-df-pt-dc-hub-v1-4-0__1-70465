...
7:      With TrayIcon
8:        .cbSize = Len(TrayIcon) 'This size is always the len(NOTIFYICONDATA)
9:        .hwnd = Owner.hwnd  'Which form is this icon for
10:        .uID = luID
11:        .uFlags = NIF_ICON Or NIF_TIP Or NIF_MESSAGE    'set valid data inputs
...
21:        .uCallbackMessage = WM_TRAYHOOK
22:        .szTip = Trim(ToolTip$) & vbNullChar   'set the tooltip
23:        If tIcon Is Nothing Then
24:            .hIcon = Owner.Icon
25:        Else
26:            .hIcon = tIcon
27:        End If
...
31:    CreateTrayIcon = Shell_NotifyIcon(NIM_ADD, TrayIcon)
32:  Exit Function

34:
Err:
35:    HandleError Err.Number, Err.Description, Erl & "|" & "mSysTray.CreateTrayIcon()"
End Function
...
7:    With TrayIcon
8:        .cbSize = Len(TrayIcon)
9:        .hwnd = Owner.hwnd
10:        .uID = luID
11:        .uFlags = NIF_ICON Or NIF_TIP Or NIF_MESSAGE
12:        .uCallbackMessage = WM_TRAYHOOK
13:        If Not tIcon Is Nothing Then
14:            .hIcon = tIcon
15:        End If
16:        If Not ToolTip = "" Then .szTip = Trim(ToolTip$) & vbNullChar
17:    End With
    'Update the tray icon with an API call
19:    ModifyTrayIcon = Shell_NotifyIcon(NIM_MODIFY, TrayIcon)
20:  Exit Function
21:
Err:
22:  HandleError Err.Number, Err.Description, Erl & "|" & "mSysTray.ModifyTrayIcon()"
End Function
...
12:   DeleteTrayIcon = Shell_NotifyIcon(NIM_DELETE, TrayIcon)
13:  Exit Function
14:
Err:
...
15:
Err:
16:    HandleError Err.Number, Err.Description, Erl & "|" & "mSysTray.InsertHook()"
End Function
...
6:    If lHookID Then    'Make sure we really are hooked
7:        SetWindowLong Owner.hwnd, GWL_WNDPROC, lHookID
        'bIsHooked = False
9:    End If
...
12:
Err:
13:  HandleError Err.Number, Err.Description, Erl & "|" & "mSysTray.RemoveHook()"
End Sub
...
7:    If shWnd = frmHub.hwnd Then
8:        GlobalMessageCatcher = frmHub.WindowProcSysTray(shWnd, uMsg, wParam, lParam)
9:    End If
...
13:
Err:
14:   HandleError Err.Number, Err.Description, Erl & "|" & "mSysTray.GlobalMessageCatcher()"
End Function
...
8:    On Error GoTo Err
    ...
    'new ones appear
13:    Call RemoveBalloon(Owner, luID)
14:    With TrayIcon
15:         .cbSize = Len(TrayIcon)
16:        .hwnd = Owner.hwnd
17:        .uID = luID
18:        .uFlags = NIF_ICON Or NIF_INFO Or NIF_MESSAGE Or NIM_MODIFY
19:        .uCallbackMessage = WM_TRAYHOOK
20:        If tIcon Is Nothing Then
21:            .hIcon = Owner.Icon
22:        Else
23:            .hIcon = tIcon
24:        End If
25:        .dwState = 0
26:        .dwStateMask = 0
27:        .szInfo = Message & Chr(0)
28:        .szInfoTitle = Title & Chr(0)
        Select Case IconType
              Case ICON_NONE
29:                .dwInfoFlags = NIIF_NONE
              Case ICON_INFO
30:                .dwInfoFlags = NIIF_INFO
              Case ICON_WARNING
31:                .dwInfoFlags = NIIF_WARNING
              Case ICON_ERROR
32:                .dwInfoFlags = NIIF_ERROR
              Case Icon_PTDCH
33:                .dwInfoFlags = NIIF_PTDCH
34:           End Select
...
41:
Err:
42:    HandleError Err.Number, Err.Description, Erl & "|" & "mSysTray.PopupBalloon()"
End Function
...
6:        .hwnd = Owner.hwnd
7:        .uID = luID
8:        .uFlags = NIF_ICON Or NIF_INFO Or NIF_MESSAGE Or NIM_MODIFY
9:        .uCallbackMessage = WM_TRAYHOOK
10:        .hIcon = Owner.Icon
11:        .dwState = 0
12:        .dwStateMask = 0
13:        .szInfo = Chr(0)
14:        .szInfoTitle = Chr(0)
15:        .dwInfoFlags = NIIF_NONE
16:    End With
17:    RemoveBalloon = Shell_NotifyIcon(NIM_MODIFY, TrayIcon)
...
4:   Dim udtGUID As GUID

6:   If (CoCreateGuid(udtGUID) = 0) Then
7:      GetGUID = _
...
23:
Err:
24:    HandleError Err.Number, Err.Description, Erl & "|" & "mSysTray.GetGUID()"
End Function
